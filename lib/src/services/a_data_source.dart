...
abstract class ADataSource {
  ...
  Future<Null> openIndexedDb(String rootIdbName) async {
    if (!idb.IdbFactory.supported)
      throw new Exception(
          "IndexedDb not suppored! Oh no!"); //TODO: Implement alternative auth caching mechanism

    if (_db == null)
      _db = await window.indexedDB.open(rootIdbName,
  ...
  @protected
  Future<dynamic> wrapTransaction(String rootIdbName, String storeName,
      String mode, Future<dynamic> toAwait(idb.ObjectStore store)) async {
    await openIndexedDb(rootIdbName);
